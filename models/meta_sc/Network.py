...
class MYNET(nn.Module):
    ...
    def forward_metric(self, x, stochastic = True):
        x_f, x_f_a, feature_a = self.encode(x)
    ...
    def encode(self, x): 
        ...
        if x.shape[1] == 44100 or x.shape[1] == 176400 or x.shape[1] == 132300:# CHECKING FOR SPECIFIC AUDIO LENGTH
        ...
        elif x.shape[1] == 220500:
            x = self.es_spectrogram_extractor(x)   # (batch_size, 1, time_steps, freq_bins)
            x = self.es_logmel_extractor(x)
        elif x.shape[1] == 110250:
            x = self.ps_spectrogram_extractor(x)   # (batch_size, 1, time_steps, freq_bins)
            x = self.ps_logmel_extractor(x) 
        ...
        x, feature = self.encoder(x)
        
        x = F.adaptive_avg_pool2d(x, 1)
        feature = F.adaptive_avg_pool2d(feature,1)
        
        x_f_a = x.squeeze(-1).squeeze(-1)#tensor with shape (batch_size, num_features)
        feature_a = feature.squeeze(-1).squeeze(-1) 
        
        x = x_f_a.unsqueeze(1)
       
        x = self.slf_attn(x, x, x)
        #print(x.shape)
        x = x.squeeze(1) # tensor with shape (batch_Size,1,  num_features)
        
        return x, x_f_a, feature_a #x_f_a represents extracted feature embeddings
    ...
    def set_fea_extractor_for_s2s(self):
        ...
        es_sample_rate = 44100
        es_window_size = 2048
        es_hop_size = 512
        es_mel_bins = 128
        es_fmax = 22050
        self.es_spectrogram_extractor = Spectrogram(n_fft=es_window_size, hop_length=es_hop_size, 
            win_length=es_window_size, window="hann", center=center, pad_mode=pad_mode, 
            freeze_parameters=True)

        # Logmel feature extractor
        self.es_logmel_extractor = LogmelFilterBank(sr=es_sample_rate, n_fft=es_window_size, 
            n_mels=es_mel_bins, fmin=0, fmax=es_fmax, ref=ref, amin=amin, top_db=top_db, 
            freeze_parameters=True)
        
        ps_sample_rate = 22050
        ps_window_size = 2048
        ps_hop_size = 1024
        ps_mel_bins = 128
        ps_fmax = 11025
        self.ps_spectrogram_extractor = Spectrogram(n_fft=ps_window_size, hop_length=ps_hop_size, 
            win_length=ps_window_size, window="hann", center=center, pad_mode=pad_mode, 
            freeze_parameters=True)

        # Logmel feature extractor
        self.ps_logmel_extractor = LogmelFilterBank(sr=ps_sample_rate, n_fft=ps_window_size, 
            n_mels=ps_mel_bins, fmin=0, fmax=ps_fmax, ref=ref, amin=amin, top_db=top_db, 
            freeze_parameters=True)
