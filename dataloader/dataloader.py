...
def get_testloader(args, session):
    ...
    if 'nsynth' in args.dataset:
        ...
        testset = args.Dataset.LBRS(root=args.dataroot, phase="test",
                                      index=class_new, k=None, args=args)
    elif 'esc' in args.dataset:
        testset = args.Dataset.ESC(root=args.dataroot, phase="test",
                                      index=class_new, k=None, args=args)
    elif 'pqrs' in args.dataset:
        testset = args.Dataset.ESCS(root=args.dataroot, phase="test",
                                      index=class_new, k=None, args=args)
...
def get_pretrain_dataloader(args):
    ...
    if 'nsynth' in args.dataset:
        ...
        trainset = args.Dataset.ESC(root=args.dataroot, phase="train",
                                             index=class_index, base_sess=True, args=args)
        valset = args.Dataset.ESC(root=args.dataroot, phase="val", index=class_index, base_sess=True, args=args)
    elif 'pqrs' in args.dataset:
        trainset = args.Dataset.ESCS(root=args.dataroot, phase="train",
                                             index=class_index, base_sess=True, args=args)
        valset = args.Dataset.ESCS(root=args.dataroot, phase="val", index=class_index, base_sess=True, args=args)
...
def get_dataset_for_data_init(args):
    ...
    if 'nsynth' in args.dataset:
        ...
        trainset = args.Dataset.ESC(root=args.dataroot, phase='train', index=class_index, k=None, args=args)
    elif 'pqrs' in args.dataset:
        trainset = args.Dataset.ESCS(root=args.dataroot, phase='train', index=class_index, k=None, args=args)
...
def get_base_dataloader_meta(args):
    ...
    if 'nsynth' in args.dataset:
        trainset = args.Dataset.NDS(root=args.dataroot, phase='train', index=class_index, k=None, args=args)
        # print(' sess args.dataroot',args.dataroot)
        # print('seee trainset.targets',trainset.targets)
        ...
        trainset = args.Dataset.ESC(root=args.dataroot, phase='train', index=class_index, k=None, args=args)
        valset = args.Dataset.ESC(root=args.dataroot, phase='val', index=class_index, k=100, args=args) # k is same as new_loader's testset k
    elif 'pqrs' in args.dataset:
        trainset = args.Dataset.ESCS(root=args.dataroot, phase='train', index=class_index, k=None, args=args)
        valset = args.Dataset.ESCS(root=args.dataroot, phase='val', index=class_index, k=100, args=args) # k is same as new_loader's testset k
    ...
    # print("(((((#########3)))))inmsideeee",trainset.targets)
...
def get_new_dataloader(args, session):
    ...
    if args.dataset == 'FMC':
        ...
        #print("root: ",args)
        session_classes = np.arange(num_base_class + (session -1) * args.way, num_base_class + session * args.way)
        # print('session_classes',session_classes)
        ...
        session_classes = np.arange(num_base_class + (session -1) * args.way, num_base_class + session * args.way)
        trainset = args.Dataset.ESC(root=args.dataroot, phase='train', index=session_classes, k=None, args=args)
    elif 'pqrs' in args.dataset:
        session_classes = np.arange(num_base_class + (session -1) * args.way, num_base_class + session * args.way)
        trainset = args.Dataset.ESCS(root=args.dataroot, phase='train', index=session_classes, k=None, args=args)
    ...
    if 'esc' in args.dataset:
        valset = args.Dataset.ESC(root=args.dataroot, phase='val', index=class_new, k=None, args=args)   
    if 'pqrs' in args.dataset:
        valset = args.Dataset.ESCS(root=args.dataroot, phase='val', index=class_new, k=None, args=args)   
